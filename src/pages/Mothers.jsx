...
export default function Mothers({ user, onRegister }) {
  ...
  const visibleMothers = isDemoAccount(user) ? mothers : []
  ...
  return <div>
    ...
    <div className="list-footer"><span><span className="footer-dot" />Care list synced just now</span><span>{isDemoAccount(user) ? <>Showing demo data · <button onClick={() => showToast({ title: 'Export ready', message: 'Your care list is ready to download.' })}>Export list <ArrowUpRight size={13} /></button></> : 'No records yet'}</span></div>
...
export function RegisterMother({ onBack, onRegistered }) {
  const { t } = useLanguage()
  const [form, setForm] = useState({ fullName: '', age: '', village: 'Kanyanya Parish', phone: '', weeks: '', gravida: '', parity: '', contact: '', bloodGroup: '', edd: '', history: '', allergies: '', complications: '' })
  ...
  function submit(event) {
    ...
    if (!Object.keys(nextErrors).length) onRegistered(form)
  }
  return <div>
