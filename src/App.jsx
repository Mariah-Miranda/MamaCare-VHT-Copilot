...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (!token) { localStorage.removeItem('mamacare_user'); setCheckingSession(false); return }
    authApi.me().then(({ data }) => {
      const nextUser = data.user
      setUser({ ...nextUser, firstName: nextUser.name.split(' ')[0], initials: nextUser.name.split(' ').map((part) => part[0]).slice(0, 2).join('').toUpperCase() })
    }).catch(() => { localStorage.removeItem('mamacare_token'); localStorage.removeItem('mamacare_user') }).finally(() => setCheckingSession(false))
  }, [])
